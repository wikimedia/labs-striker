    ...
    def __init__(self, *args, **kwargs):
        ...
        if len(toolinfo) == 1 and not toolinfo[0].repository:
            self.fields["mark_as_toolinfo_repository"] = forms.BooleanField(
                label=_("Set source code repository in toolinfo to this repository"),
                help_text=_(
...
class ProjectCreateForm(forms.Form):
    def __init__(self, *args, **kwargs):
        ...
        if len(toolinfo) == 1:
            if toolinfo[0].description:
                self.fields["project_description"].initial = toolinfo[0].description
            ...
            if toolinfo[0].repository:
                self.fields["source_code_repository"].initial = toolinfo[0].repository
    ...
    def clean_project_name(self):
        ...
        try:
            ...
            raise forms.ValidationError(
                _('Project "%(name)s" exists.'), params={"name": name}, code="duplicate"
            )
        except KeyError:
            return name
...
class AccessRequestAdminForm(AccessRequestCommentForm):
    ...
    class Meta(AccessRequestCommentForm.Meta):
        ...
        labels = {
            "suppressed": _("Suppress this request (hide from non-admin users)"),
        }
...
class ToolInfoForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if "license" in self.fields:
            ...
            license.empty_label = _("-- Choose your software license --")
            license.queryset = SoftwareLicense.objects.filter(
                osi_approved=True
            ).order_by("-recommended", "slug")
...
class ToolCreateForm(forms.Form):
    ...
    name = forms.CharField(
        label=_("Tool name"),
        help_text=_(
            "The tool name is used as part of the URL for the tool's webservice."
        ),
        widget=forms.TextInput(
            attrs={
                "autofocus": "autofocus",
                ...
                "data-parsley-remote": mark_safe("/tools/api/toolname/{value}"),
    ...
    description = forms.CharField(
        ...
        widget=forms.Textarea(
            attrs={
                "placeholder": _("A short summary of what your tool will do"),
                "rows": 5,
            },
        ),
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["tags"].queryset = ToolInfoTag.objects.all().order_by("name")

...
def create(req, tool):
    ...
    if req.method == "POST":
        if form.is_valid():
            ...
            try:
                phab_maintainers += [
                    m["phid"] for m in phab.user_ldapquery(maintainers)
                ]
            except KeyError:
                pass
            try:
                phab_maintainers += [
                    m["phid"]
                    for m in phab.user_mediawikiquery(
                        list(
                            LabsUser.objects.filter(
            ...
            if phab_maintainers:
                ...
                project = phab.create_project(
                    ...
                    repository=form.cleaned_data.get("source_code_repository", None),
                )

                # Save a local association between the project and the tool.
                project_model = PhabricatorProject(
                    tool=tool.name, name=name, phid=project["phid"], created_by=req.user
                )
                try:
                    project_model.save()
                    try:
                    ...
                    if form.cleaned_data.get("mark_as_toolinfo_issue_tracker", False):
                        toolinfo = tool.toolinfo().get()
                        with reversion.create_revision():
                            reversion.set_user(req.user)
                            reversion.set_comment(
                                "set issue tracker to created Phabricator project"
                            )

...
class SshKeyDeleteForm(forms.Form):
    ...
    def clean_key_hash(self):
        ...
        if key_hash not in hashes:
            raise forms.ValidationError(_("SSH key not found."), code="key_not_found")
        self.cleaned_keys = [key for (hash, key) in hashes.items() if hash != key_hash]
        return key_hash
...
class SshKeyForm(forms.Form):
    ...
    def clean_public_key(self):
        ...
        if key is None:
            ...
            raise forms.ValidationError(_("Invalid public key."), code="key_invalid")
        if pub_key in self.keys:
            raise forms.ValidationError(
                _("Public key {hash} already in use.").format(hash=key.hash_sha256()),
                code="key_duplicate",
            )
        self.key = key
        return pub_key

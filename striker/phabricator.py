...
class Client(object):
    ...
    def post(self, path, data):
        ...
        if resp["error_code"] is not None:
            raise APIError(
                resp["error_info"], resp["error_code"], resp.get("result", None)
            )
        return resp["result"]
    ...
    def user_ldapquery(self, names):
        ...
        try:
            ...
            if (
                ...
                if e.result is None:
                    raise KeyError("Users not found for {0}".format(", ".join(names)))
                else:
    ...
    def get_policies(self, phids):
        ...
        try:
            ...
            if e.code == "ERR-INVALID-PARAMETER" and "Unknown policies" in e.message:
                logger.warning(e.message)
                if e.result is None:
                    raise KeyError(
    ...
    def create_project(self, name, *, members, parent, description, repository=None):
